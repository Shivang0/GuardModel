...
function generateMarkdown(results) {
    ...
    if (findingsToShow.length > 0) {
        ...
        for (const { finding, result } of findingsToShow) {
            const icon = (0, orchestrator_1.getSeverityIcon)(finding.severity);
...
function generateSummary(results) {
    if (results.status === 'passed') {
        ...
        const threats = [];
        if (results.critical > 0)
            threats.push(`${results.critical} critical`);
        if (results.high > 0)
            threats.push(`${results.high} high`);
        if (results.medium > 0)
            threats.push(`${results.medium} medium`);
